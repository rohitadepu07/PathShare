  ...
  return (
    <div className="flex-1 flex flex-col bg-white dark:bg-slate-950 transition-colors">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-24 no-scrollbar">
        ...
        <div className="mt-10">
          <div className="flex justify-between items-center mb-4">
            <h4 className="text-lg font-bold dark:text-white">Earn Points Fast</h4>
            <button className="text-[#0d828c] font-semibold text-sm">See All</button>
          </div>
          <div className="flex gap-4 overflow-x-auto no-scrollbar py-2">
            {[
              { label: 'City Bike', icon: 'fa-motorcycle', color: 'text-orange-600 dark:text-orange-400', bg: 'bg-orange-50 dark:bg-orange-900/20' },
              { label: 'Office Car', icon: 'fa-briefcase', color: 'text-[#0d828c] dark:text-[#11a4b1]', bg: 'bg-[#0d828c]/10 dark:bg-[#0d828c]/20' },
              { label: 'Eco EV', icon: 'fa-bolt', color: 'text-green-600 dark:text-green-400', bg: 'bg-green-50 dark:bg-green-900/20' },
            ].map((cat, idx) => (
              <div 
                key={idx} 
                onClick={onSearchClick}
                className={`${cat.bg} min-w-[120px] p-4 rounded-3xl flex flex-col items-center gap-3 border border-transparent hover:border-gray-200 dark:hover:border-slate-700 cursor-pointer active:scale-95 transition-all`}
              >
                <div className={`text-2xl ${cat.color}`}>
                  <i className={`fa-solid ${cat.icon}`}></i>
                </div>
                <span className="text-sm font-bold text-gray-700 dark:text-slate-300">{cat.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl border-t border-gray-100 dark:border-slate-900 p-4 flex justify-around items-center absolute bottom-0 w-full rounded-t-[32px] shadow-[0_-8px_30px_rgb(0,0,0,0.06)] dark:shadow-[0_-8px_30px_rgb(0,0,0,0.3)] z-20">
        ...
        <button onClick={onOpenSidebar} className="text-slate-400 flex flex-col items-center gap-1 active:scale-90 transition-transform">
