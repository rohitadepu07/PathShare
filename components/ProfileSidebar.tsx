...
const ProfileSidebar: React.FC<ProfileSidebarProps> = ({ 
  ...
  const menuItems = [
    ...
    { label: 'My Impact Trips', icon: 'fa-route', action: () => {} },
      ...
      >
        ...
        {isLoggedIn ? (
          <div className="flex-1 flex flex-col overflow-y-auto no-scrollbar px-6">
            <div className="bg-[#F8FAFC] dark:bg-slate-800/50 rounded-[40px] p-7 mb-6 border border-gray-100/50 dark:border-slate-800">
              <div className="flex items-start gap-5 mb-5">
                <div className="relative">
                  <img src={user.avatar} className="w-20 h-20 rounded-[28px] object-cover shadow-sm ring-4 ring-white dark:ring-slate-800" alt="Avatar" />
                ...
                <div className="flex-1 pt-1">
                  <div className="flex items-center justify-between">
                    <h3 className="font-bold text-[22px] text-slate-900 dark:text-white tracking-tight leading-tight">{user.name}</h3>
                    <button onClick={() => setIsEditing(!isEditing)} className="text-slate-300 hover:text-[#0d828c] transition-colors">
                      <i className={`fa-solid ${isEditing ? 'fa-xmark' : 'fa-pen-to-square'} text-sm`}></i>
                    </button>
                  </div>
              ...
              {isEditing ? (
                <div className="space-y-3 animate-fade-in">
                  <textarea 
                    className="w-full bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl p-4 text-sm text-slate-600 dark:text-slate-300 resize-none outline-none focus:border-[#0d828c] transition-all"
                    value={editForm.bio}
                    rows={3}
                    onChange={(e) => setEditForm({ ...editForm, bio: e.target.value })}
                  />
                  <button onClick={handleSave} disabled={isSaving} className="w-full bg-[#0d828c] text-white py-4 rounded-2xl text-sm font-bold shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all">
                    {isSaving ? <i className="fa-solid fa-circle-notch animate-spin"></i> : 'Update Bio'}
                  </button>
                </div>
              ) : (
                <>
                  <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed mb-6">{user.bio}</p>
                  <div className="flex gap-2">
                    <div className="flex-1 bg-white dark:bg-slate-800 p-3 rounded-2xl border border-slate-100 dark:border-slate-700 text-center">
                        <div className="text-lg font-black text-slate-900 dark:text-white">1.2k</div>
                        <div className="text-[8px] font-bold text-slate-400 uppercase">Points</div>
                    </div>
                    <div className="flex-1 bg-white dark:bg-slate-800 p-3 rounded-2xl border border-slate-100 dark:border-slate-700 text-center">
                        <div className="text-lg font-black text-slate-900 dark:text-white">42</div>
                        <div className="text-[8px] font-bold text-slate-400 uppercase">Rides</div>
                    </div>
                  </div>
                </>
              )}
